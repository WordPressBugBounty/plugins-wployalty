...
$wlrf_heading_color        = Settings::get( 'heading_color', '#1D2327' );
$wlrf_background_color     = Settings::get( 'background_color', '#ffffff' );
$wlrf_button_text_color    = Settings::get( 'button_text_color', '#ffffff' );
...
<div class="wlr-myaccount-page <?php echo esc_attr( ! empty( $page_type ) ? 'wlr-page-' . $page_type : '' ); ?>">
    ...
    <!--User Card start-->
	<?php if ( $wlrf_is_user_available || get_current_user_id() ): ?>
                ...
                <div id="<?php echo esc_attr( WLR_PLUGIN_PREFIX . '-points' ) ?>">
                    <div id="<?php echo esc_attr( WLR_PLUGIN_PREFIX . '-available-points' ); ?>"
                        ...
                        <div>
							...
							<?php
							// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Existing public WPLoyalty hook retained because customers may use it.
							do_action("wlr_after_customer_reward_page_available_points_content"); ?>
                        </div>
                        <div>
							<?php $wlrf_user_points = (int) ( ! empty( $user ) && ! empty( $user->points ) ? $user->points : 0 ); ?>
                            <span id="<?php echo esc_attr( WLR_PLUGIN_PREFIX . '-available-points-heading' ); ?>"
                                  class="wlr-text-color">
        <?php echo /* translators: %s: label */
        esc_html( sprintf( __( 'Available %s', 'wp-loyalty-rules' ), $wlrf_earn_campaign_helper->getPointLabel( $wlrf_user_points ) ) ) ?></span>
                    ...
                    <div id="<?php echo esc_attr( WLR_PLUGIN_PREFIX . '-redeemed-points' ) ?>" class="wlr-border-color">
                        ...
                        <div>
							<?php $wlrf_user_total_points = (int) ( ! empty( $user ) && ! empty( $user->used_total_points ) ? $user->used_total_points : 0 ); ?>
                                    ...
                                    <select id="wlr_currency_list" class="wlr-border-color wlr-text-color"
										...
										<?php foreach ( $current_currency_list as $wlrf_currency_key => $wlrf_currency_label ): ?>
                                            <option value="<?php echo esc_attr( $wlrf_currency_key ); ?>"
												<?php echo ( ! empty( $current_currency ) && ( $wlrf_currency_key === $current_currency ) ) ? "selected" : ""; ?>><?php echo esc_html( $wlrf_currency_key ); ?></option>
										<?php endforeach; ?>
                                    </select>
								<?php endif; ?>
                            </div>
                    ...
                    <div class="wlr-level-data-section">
                        <div class="wlr-level-content">
							...
							if ( isset( $user->level_data->current_level_start ) && isset( $user->level_data->next_level_start ) && $user->level_data->next_level_start > 0 ):
								// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Existing public WPLoyalty hook retained because customers may use it.
								$wlrf_points = apply_filters( 'wlr_points_for_my_account_reward_page', $user->earn_total_point, $user );
								$wlrf_css_width = ( ( $wlrf_points - $user->level_data->current_level_start ) / ( $user->level_data->next_level_start - $user->level_data->current_level_start ) ) * 100;
								$wlrf_needed_point = $user->level_data->next_level_start - $wlrf_points;
								?>
                ...
                <div class="wlr-social-share">
					<?php foreach ( $social_share_list as $wlrf_action => $wlrf_social_share ): ?>
                        <a class="wlr-icon-list"
                           ...
                           target="_parent">
							<?php $wlrf_social_icon = ! empty( $wlrf_social_share['icon'] ) ? $wlrf_social_share['icon'] : "";
							$wlrf_social_image_icon = ! empty( $wlrf_social_share['image_icon'] ) ? $wlrf_social_share['image_icon'] : "social";
							?>
	...
	echo ! empty( $my_reward_section ) ? $my_reward_section : ''; ?>
	...
	<?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	echo ! empty( $trans_details ) ? $trans_details : ''; ?>
				...
				foreach ( $campaign_list as $wlrf_campaign ) : ?>
					<?php if ( isset( $wlrf_campaign->is_show_way_to_earn ) && $wlrf_campaign->is_show_way_to_earn == 1 ): ?>
                        <div class="wlr-card wlr-earning-option wlr-border-color">
							<?php if ( ! empty( $wlrf_campaign->level_batch ) && is_array( $wlrf_campaign->level_batch ) ): ?>
                                <div class="wlr-campaign-level-batch">
									...
									foreach ( $wlrf_campaign->level_batch as $wlrf_batch_label ):
										if ( $wlrf_check_level_count > 2 ): ?>
                                            <span class="wlr-text-color wlr-border-color"><?php echo esc_html( sprintf( '+%s', $wlrf_campaign->level_batch_count_show ) ); ?></span>
											<?php break;
                            ...
                            <div class="wlr-card-container">
								<?php $wlrf_action_type = ! empty( $wlrf_campaign->action_type ) ? $wlrf_campaign->action_type : ""; ?>
								<?php $wlrf_img_icon = ! empty( $wlrf_campaign->icon ) ? $wlrf_campaign->icon : ""; ?>
                                ...
                                <div style="display: flex;align-items: center;gap:5px;justify-content: space-between;">
									<?php if ( ! empty( $wlrf_campaign->campaign_title_discount ) ) : ?>
									...
									<?php endif; ?>
									<?php if ( ( $wlrf_is_user_available || get_current_user_id() > 0 ) && isset( $wlrf_campaign->action_type ) && $wlrf_campaign->action_type == 'followup_share' ) : ?>
										...
										$wlrf_share_url        = ! empty( $wlrf_point_rule->share_url ) ? $wlrf_point_rule->share_url : ''; ?>
                                        <div class="wlr-date wlr-followup-section"
                                             style="position:relative;border-radius: 6px;padding:4px;background: <?php echo esc_attr( $wlrf_theme_color ); ?>;<?php echo $wlrf_is_right_to_left ? "float: left;" : "float:right;"; ?>">
									...
									<?php if ( isset( $wlrf_campaign->action_type ) && $wlrf_campaign->action_type == 'birthday' ) : ?>
										...
										$wlrf_birth_date                 = ! empty( $user->birthday_date ) && $user->birthday_date != '0000-00-00' ? $wlrf_woocommerce_helper->convertDateFormat( $user->birthday_date ) : ( ! empty( $user->birth_date ) ? $wlrf_woocommerce_helper->beforeDisplayDate( $user->birth_date ) : '' );
										...
										// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Existing public WPLoyalty hook retained because customers may use it.
										$wlrf_show_edit_birthday         = apply_filters( "wlr_allow_my_account_edit_birth_date", $wlrf_show_edit_birthday, $wlrf_user_can_edit_birthdate, ! empty( $user ) ? $user : new \stdClass() );
										?>
								...
								<?php if ( is_object( $wlrf_campaign ) && ! empty( $wlrf_campaign->description ) && $wlrf_campaign->description != 'null' ) : ?>
									<?php //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
									echo \Wlr\App\Helpers\Base::readMoreLessContent( $wlrf_campaign->description, $wlrf_card_key, 90, __( "Show more", "wp-loyalty-rules" ), __( "Show less", "wp-loyalty-rules" ), 'card-campaign-description', 'wlr-description wlr-pre-text wlr-text-color' ); ?>
								<?php endif; ?>
                            </div>
                        </div>
						<?php $wlrf_card_key ++;
            ...
            <div class="wlr-campaign-container">
				...
				foreach ( $reward_list as $wlrf_reward ) : ?>
					<?php if ( isset( $wlrf_reward->is_show_reward ) && $wlrf_reward->is_show_reward == 1 ): ?>
                        <div class="wlr-card wlr-earning-option wlr-border-color">
                            <div class="wlr-card-container">
								<?php $wlrf_discount_type = ! empty( $wlrf_reward->discount_type ) ? $wlrf_reward->discount_type : "" ?>
								<?php $wlrf_img_icon = ! empty( $wlrf_reward->icon ) ? $wlrf_reward->icon : "" ?>
								<?php echo wp_kses_post( \Wlr\App\Helpers\Base::setImageIcon( $wlrf_img_icon, $wlrf_discount_type, array( "alt" => $wlrf_reward->name ) ) ); ?>
                                <h4 class="wlr-name">
									<?php //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
									echo \Wlr\App\Helpers\Base::readMoreLessContent( $wlrf_reward->name, $wlrf_card_key, 60, esc_html__( "Show more", "wp-loyalty-rules" ), esc_html__( "Show less", "wp-loyalty-rules" ), 'card-ways-to-earn-name', 'wlr-name wlr-pre-text wlr-text-color' ); ?>
                                </h4>
								<?php if ( ! empty( $wlrf_reward->description ) && $wlrf_reward->description != 'null' ) : ?>
									<?php //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
									echo \Wlr\App\Helpers\Base::readMoreLessContent( $wlrf_reward->description, $wlrf_card_key, 90, esc_html__( "Show more", "wp-loyalty-rules" ), esc_html__( "Show less", "wp-loyalty-rules" ), 'card-ways-to-earn-description', 'wlr-description wlr-pre-text wlr-text-color' ); ?>
								<?php endif; ?>
                            </div>
                        </div>
	...
	<?php if ( ( isset( $is_sent_email_display ) && $is_sent_email_display === 'yes' ) && $wlrf_is_user_available ): ?>
